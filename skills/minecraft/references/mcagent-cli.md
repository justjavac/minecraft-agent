# mcagent CLI Reference

Use this file only as an offline fallback. Prefer `mcagent skills get core` because it is generated by the installed CLI version.
...
If `mcagent` is missing, install the npm package first:
...
mcagent --output json build place-line --session default --from-x <x1> --from-y <y1> --from-z <z1> --to-x <x2> --to-y <y2> --to-z <z2> --face up --item dirt --max-blocks 64
